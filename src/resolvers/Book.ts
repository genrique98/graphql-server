...
export const Book = {
    author: async(parent: Book_model, args: Author_model, context: Context, info: Author_model): Promise<Author_model[]> => {
        try {
    ...
    publisher: async(parent: Book_model, args: Publisher_model, context: Context, info: Publisher_model): Promise<Publisher_model[]> => {
        try {
            const conn = await Client.connect();
